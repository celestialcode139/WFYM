...
function Generalinfo(props: any) {
  ...
  const handleUpdatePoliticalParty = (value) => {
    ...
    setErrorState(UpdatedErrorState);
    setbody({ ...body, political_Party: value });
  };
  const handleUpdateChildrensBefore = (value) => {
    const UpdatedErrorState = errorState.filter(
      (item) => item !== "children_before"
    );
    setErrorState(UpdatedErrorState);
    setbody({ ...body, beforeChildren: value });
  };
    ...
    <Box sx={{ paddingLeft: "0px" }}>
      <Grid container spacing={2}>
        <Grid item sm={12}>
          ...
          {/* <Box sx={{ marginBottom: "30px", width: "200px" }}>
            <RulerInput />
          </Box> */}
        </Grid>
        ...
        <Grid item sm={6}>
          <Box className={`${classes.TextFieldParent}`}>
            <TextField
              ...
              label="Religion"
              variant="outlined"
              error={errorState.includes(errorFieldsKeys.idealMatch.religion)}
              SelectProps={{
                multiple: false,
                value: body.religion,
        ...
        <Grid item md={6}>
          <Box className={`${classes.TextFieldParent}`}>
            <TextField
              ...
              error={errorState.includes(errorFieldsKeys.idealMatch.location)}
              label="City"
              value={body.location}
              onChange={(e) => {
          ...
          <Box className={`${classes.TextFieldParent}`}>
            <TextField
              ...
              error={errorState.includes(
                errorFieldsKeys.idealMatch.political_party
              )}
              SelectProps={{
                multiple: false,
          ...
          <Grid item md={6}>
            <Box className={`${classes.TextFieldParent}`}>
              <TextField
                ...
                type="number"
                error={errorState.includes(
                  errorFieldsKeys.idealMatch.children_before
                )}
                onChange={(e) => {
                  handleUpdateChildrensBefore(e.target.value);
                }}
              />
            </Box>
          </Grid>
        ...
        <Grid item md={12}>
          <Box className={`${classes.TextFieldParent}`}>
            <TextField
              ...
              type="text"
              label="Description"
              placeholder="Tell me about your match"
              multiline
              rows={4}
              value={body.description}
              error={errorState.includes(
                errorFieldsKeys.idealMatch.description
              )}
