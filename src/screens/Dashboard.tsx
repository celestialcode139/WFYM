...
function Dashboard() {
    ...
    APIHelper.CallApi(
      ...
      if (result.status == "success") {
        ...
        if (result.data.isProfileVerified.status == false) {
          ...
          NavigateToProfileUpdate(result.data.isProfileVerified.fields ?? []);
        } else if (result.data.isIdealPersonVerified.status == false) {
          Alert.notify(
            `Compleate Your Ideal Match Profile. ${
              result.data.isIdealPersonVerified.msg ?? ""
            }`,
            3000
          );
          setErrorState(result.data.isIdealPersonVerified.fields ?? []);
          setRequestMatchLoading(false);
          console.log(
            "Uncompleate Fields ",
            result.data.isIdealPersonVerified.fields
          );
          NavigateToIdealMatchUpdate(
            result.data.isIdealPersonVerified.fields ?? []
          );
  ...
  const NavigateToProfileUpdate = (array) => {
    ...
    const secondPageItems = ["hobbies"];
    const fourthPageItems = ["personality"];
    const fifthPageItems = [
      "profession",
      "religion",
      "political_party",
      "children_before",
    ...
    if (array.some((item) => firstPageItems.includes(item))) {
      NavigateTo("/profile/page-1");
    } else if (array.some((item) => secondPageItems.includes(item))) {
      NavigateTo("/profile/page-2");
    } else if (array.some((item) => fourthPageItems.includes(item))) {
      NavigateTo("/profile/page-4");
    } else if (array.some((item) => fifthPageItems.includes(item))) {
      NavigateTo("/profile/page-5");
    } else {
      NavigateTo("/profile/page-1");
    }
  };
  const NavigateToIdealMatchUpdate = (array) => {
    const firstPageItems = [
      "religion",
      "location",
      "political_party",
    ...
    const secondPageItems = ["looking_for"];
    const thirdPageItems = ["personality"];
    // const fifthPageItems = [
    //   "profession",
    //   "religion",
    //   "political_party",
    ...
    if (array.some((item) => firstPageItems.includes(item))) {
      ...
      NavigateTo("/ideal-personality/looking-for");
    } else if (array.some((item) => thirdPageItems.includes(item))) {
      NavigateTo("/ideal-personality/looks");
    }
    //  else if (array.some((item) => fifthPageItems.includes(item))) {
    //   NavigateTo("/profile/page-5");
    // }
                    ...
                    matchFavourite.map((favourite: any, i: number) => (
                      <Grid item xs={6} key={i}>
                        <MatchCards
                          ...
                          img={favourite.result_user_id?.user_details?.images}
          ...
          <Grid item xs={12} md={currentMatches?.length > 0 ? 5 : 8.5}>
            ...
            >
              <Box
                className="sticky"
                sx={{
                  display: currentMatches?.length > 0 ? "block" : "none",
                }}
              >
