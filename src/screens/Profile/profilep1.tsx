...
function ProfileP1() {
	...
	const { errorState } = useAuth();
	console.log("errorState", errorFieldsKeys.profile.country);
	...
	const GetProfile = async (Token: string) => {
		APIHelper.CallApi(
			...
			if (result.status == "success") {
				...
				setLastName(
					result?.data?.last_name ? result.data.last_name : ""
				);
	...
	const UpdateBio = () => {
		...
		APIHelper.CallApi(
			config.Endpoints.user.UpdateBio,
			data,
			null,
			Token
		).then((result) => {
							...
							<Grid item md={6} xs={12} sx={{ p: 1 }}>
								<TextField
									...
									error={errorState.includes(
										errorFieldsKeys.profile.country
									)}
									type="text"
									label="Country"
									value={Country}
						...
						<Grid container>
							...
							<Grid item xs={12} sx={{ p: 1 }}>
								<TextField
									...
									error={errorState.includes(
										errorFieldsKeys.profile.location
									)}
								/>
